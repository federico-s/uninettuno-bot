...
async function setControlliVelocita(){
	...
	chrome.storage.sync.get("currentPlaybackRate", function(items){
		...
		controls.innerHTML="Velocit&agrave;: <input type=\"number\" value=\""+currPbRt+"\" min=\"0.25\" max=\"3\" step=\"0.25\" id=\"speedInput\">";
...
window.addEventListener("load", setControlliVelocita().then(function load(event){
	    var speedInput = document.getElementById('speedInput');

	    speedInput.addEventListener('change', function(e) { 

	    	chrome.storage.sync.set({"currentPlaybackRate": e.target.value}, function(){
